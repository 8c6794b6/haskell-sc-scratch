...
class Pval p where
  pval :: (Binary a, Show a) => a -> p a

class Prepeat p where
  prepeat :: (Binary a, Show a) => a -> p a

-- | Make pattern from list.
class Plist p where
  plist :: (Binary a, Show a) => [a] -> p a
...
class Plc p where
  pz  :: p (a,h) a
  ps  :: p h a -> p (any,h) a
  lam :: p (a,h) b -> p h (a->p h b)
  app :: p h (a->p h b) -> p h a -> p h b
