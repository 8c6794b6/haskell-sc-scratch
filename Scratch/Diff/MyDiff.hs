...
best :: EditScriptL f txs tys -> EditScriptL f txs tys -> EditScriptL f txs tys
best dx dy = l `par` (r `seq` bestSteps l dx r dy) where
  l = steps dx
  r = steps dy
...
-- best :: EditScriptL f txs tys -> EditScriptL f txs tys -> EditScriptL f txs tys
-- best dx dy = r `par` (l `pseq` bestSteps l dx r dy) where
--   r = steps dx
--   l = steps dy

steps :: EditScriptL f txs tys -> Int
steps (Ins _ d) = succ $ steps d
steps (Del _ d) = succ $ steps d
steps (Cpy _ d) = succ $ steps d
steps End       = 0

-- bestSteps :: Int -> d -> Int -> d -> d
-- bestSteps 0 x _ _ = x
-- bestSteps _ _ 0 y = y
-- bestSteps a x b y = bestSteps (pred a) x (pred b) y

bestSteps :: Int -> d -> Int -> d -> d
bestSteps a x b y | a <= b    = x
                  | otherwise = y
...
bestEditScriptLMemo cx cy isxs' isxs isys' isys i d c = case decEq cx cy of
    ...
    Nothing           -> l `par` (r `pseq` best l r) where
      l = ins isys' isys cy (getEditScriptL i)
      r = del isxs' isxs cx (getEditScriptL d)
