...
--
module Parser1 (Cmd(..), parseCmd) where
...
paramPair :: Monad m => ParsecT String u m (String,Double)
paramPair = do
  name <- generalName
  optional (many space) >> char '=' >> optional (many space)
  val <- many1 (digit <|> oneOf "-.")
  optional (many space)
  return (name, read val)
...
nodeParam = do
  name <- generalName
  many space
  params <- synthParam `sepBy` (many1 space)
  return $ (name,params)
...
toParam :: [(String,Double)] -> [SynthParam]
toParam = map (\(x,y) -> x := y)

paramName :: SynthParam -> ParamName
paramName param = case param of
  (n := _)  -> n
  (n :<- _) -> n
  (n :<= _) -> n
