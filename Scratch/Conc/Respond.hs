...
import Sound.SC3
import Sound.SC3.Lepton -- hiding (Ppar(..))
...
dumpMsg m =
  ...
  groupByS (\_ b -> getDur b == 0) =<< runPIO m
...
spanS p ss = go p ss S.empty where
  go p xs acc
    | S.null xs = (acc,xs)
    | otherwise = if p x then go p (S.tail xs) (S.snoc acc x) else (acc,xs)
    where x = S.head xs
